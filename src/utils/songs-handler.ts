...
async function songsHandler ({ guilds }: Client) {
    ...
    firestore.collection('songs').orderBy('created_at').onSnapshot(async (songs) => {
        ...
        const song = songs.docs[0] ? { document_id: songs.docs[0].id, ...songs.docs[0].data() } as FirebaseDocumentData : null
        const skipped = isSkipped(songs.docChanges()[0])

        const guild = skipped ? songs.docChanges()[0].doc.data().guild_id as string : song?.guild_id
        ...
        if (!connection && song) {
            ...
            const { id, voiceAdapterCreator, members } = await guilds.fetch(guild)
        ...
        if (connection) {
            ...
            song && !players.has(guild) && players.set(guild, playSong(connection, `https://www.youtube.com/watch?v=${song.video_id}`, (skipped) => !skipped && firestore.doc(`/songs/${song.document_id}`).delete()))
...
) => document !== undefined && document.newIndex === -1 && document.oldIndex === 0
